...
std::string make_result_key(const SweepCase &sweep_case, const fs::path &artifact_dir, int64_t npu_n) {
    std::ostringstream oss;
    oss << sweep_case.M << "x" << sweep_case.K << "x" << sweep_case.N << "__" << artifact_dir.filename().string() << "__npuN_" << npu_n;
    return oss.str();
}
...
fs::path get_default_results_path(const std::string &root_dir) {
    return fs::path(root_dir) / "py/heteroMosaic_figs/results/tensor_parallel_gemm_npu_gpu_sweepN_results.json";
}
...
std::vector<SweepPoint> discover_sweep_points(const SweepCase &sweep_case, const fs::path &artifact_dir) {
    ...
    std::map<int64_t, fs::path> pdi_by_npu_n;
    std::map<int64_t, fs::path> inst_by_npu_n;
    ...
    for (const auto &entry : fs::directory_iterator(artifact_dir)) {
        ...
        if (parse_pdi_filename(filename, file_m, file_k, file_n)) {
            if (file_m == sweep_case.M && file_k == sweep_case.K && file_n > 0 && file_n <= sweep_case.N) {
                pdi_by_npu_n[file_n] = entry.path();
            }
        } else if (parse_inst_filename(filename, file_m, file_k, file_n)) {
            if (file_m == sweep_case.M && file_k == sweep_case.K && file_n > 0 && file_n <= sweep_case.N) {
                inst_by_npu_n[file_n] = entry.path();
            }
        }
    }

    std::vector<SweepPoint> points;
    for (const auto &[npu_n, pdi_path] : pdi_by_npu_n) {
        const auto inst_it = inst_by_npu_n.find(npu_n);
        if (inst_it == inst_by_npu_n.end()) {
            continue;
        }
        points.push_back(SweepPoint {npu_n, pdi_path, inst_it->second});
    }

    std::sort(points.begin(), points.end(), [](const SweepPoint &lhs, const SweepPoint &rhs) { return lhs.npu_n < rhs.npu_n; });
    return points;
}
...
std::chrono::microseconds run_gpu_n(RuntimePointState &point_state, const torch::Tensor &input, int64_t K) {
    if (point_state.gpu_n <= 0) {
        return std::chrono::microseconds(0);
    }

    const auto start = std::chrono::high_resolution_clock::now();
    hipkernels::w4a16_gemm_fused_packed(point_state.gpu_output, input, point_state.gpu_packed_params, K, point_state.gpu_n);
    torch::cuda::synchronize();
    const auto end = std::chrono::high_resolution_clock::now();
    return std::chrono::duration_cast<std::chrono::microseconds>(end - start);
}

std::chrono::microseconds run_npu_n(RuntimePointState &point_state, const torch::Tensor &input) {
    if (point_state.npu_n <= 0) {
        return std::chrono::microseconds(0);
    }
...
void run_split_once(torch::Tensor &output, const torch::Tensor &input, RuntimePointState &point_state, int64_t K, int64_t N) {
    std::chrono::microseconds gpu_time(0);
    std::chrono::microseconds npu_time(0);

    std::optional<std::thread> gpu_thread;
    if (point_state.gpu_n > 0) {
        gpu_thread.emplace([&]() { gpu_time = run_gpu_n(point_state, input, K); });
        set_thread_affinity(*gpu_thread, GPU_THREAD_CORE);
    }

    std::optional<std::thread> npu_thread;
    if (point_state.npu_n > 0) {
        npu_thread.emplace([&]() { npu_time = run_npu_n(point_state, input); });
    ...
    if (point_state.gpu_n > 0) {
        output.slice(1, 0, point_state.gpu_n).copy_(point_state.gpu_output);
    }
    if (point_state.npu_n > 0) {
        output.slice(1, point_state.gpu_n, N).copy_(point_state.npu_output);
    }
    torch::cuda::synchronize();
}

BenchmarkStats benchmark_split(torch::Tensor &output, const torch::Tensor &input, RuntimePointState &point_state, int64_t K, int64_t N) {
    ...
    for (int iter = 0; iter < ITER; ++iter) {
        ...
        run_split_once(output, input, point_state, K, N);
...
RuntimePointState prepare_gpu_baseline_point_state(const RuntimeCaseState &case_state, const SweepCase &sweep_case) {
    RuntimePointState point_state;
    point_state.gpu_n = sweep_case.N;
    point_state.npu_n = 0;
    point_state.gpu_output =
...
RuntimePointState prepare_point_state(const RuntimeCaseState &case_state, const SweepCase &sweep_case, int64_t npu_n) {
    if (npu_n <= 0 || npu_n > sweep_case.N) {
        throw std::runtime_error("Invalid npuN " + std::to_string(npu_n));
    }

    RuntimePointState point_state;
    point_state.npu_n = npu_n;
    point_state.gpu_n = sweep_case.N - npu_n;

    if (point_state.gpu_n > 0) {
        point_state.gpu_output =
            torch::zeros({sweep_case.M, point_state.gpu_n}, torch::TensorOptions().dtype(torch::kBFloat16).device(torch::kCUDA));
        point_state.gpu_packed_params =
            build_packed_weights_from_components(case_state.qweight.slice(1, 0, point_state.gpu_n).contiguous(),
                                                 case_state.scales.slice(1, 0, point_state.gpu_n).contiguous(),
                                                 case_state.zeros.slice(1, 0, point_state.gpu_n).contiguous(), sweep_case.K, point_state.gpu_n);
    }

    point_state.npu_output =
        torch::zeros({sweep_case.M, point_state.npu_n}, torch::TensorOptions().dtype(torch::kBFloat16).device(torch::kCUDA));
    point_state.npu_packed_params =
        build_packed_weights_from_components(case_state.qweight.slice(1, point_state.gpu_n, sweep_case.N).contiguous(),
                                             case_state.scales.slice(1, point_state.gpu_n, sweep_case.N).contiguous(),
                                             case_state.zeros.slice(1, point_state.gpu_n, sweep_case.N).contiguous(), sweep_case.K,
                                             point_state.npu_n);
    torch::cuda::synchronize();

    point_state.npu_input_handle =
        import_dma_buf_to_xdna(case_state.input.data_ptr(), case_state.input.numel(), case_state.input.element_size());
    ...
    if (point_state.npu_input_handle == 0 || point_state.npu_output_handle == 0 || point_state.npu_weight_handle == 0) {
        throw std::runtime_error("Failed to import one or more N-split DMA-BUF handles to XDNA");
    }
...
void ensure_gpu_baseline_result(json &results_doc, const fs::path &results_path, const SweepCase &sweep_case, const fs::path &artifact_dir,
    ...
    try {
        RuntimePointState baseline_point_state = prepare_gpu_baseline_point_state(case_state, sweep_case);
        case_state.split_output.zero_();
        torch::cuda::synchronize();
        run_split_once(case_state.split_output, case_state.input, baseline_point_state, sweep_case.K, sweep_case.N);
        ...
        BenchmarkStats stats = benchmark_split(case_state.split_output, case_state.input, baseline_point_state, sweep_case.K, sweep_case.N);
...
int main(int argc, char **argv) {
    try {
        ...
        std::vector<SweepCase> sweep_cases = {
            {2048, 2048, 2048, true, "hw_bins/npu2/8192x2048x2048/bf16_int4AWQ_bf16_N"},
            {4096, 4096, 4096, true, "hw_bins/npu2/8192x4096x4096/bf16_int4AWQ_bf16_N"},
            {8192, 8192, 8192, true, "hw_bins/npu2/8192x8192x8192/bf16_int4AWQ_bf16_N"},
            {8192, 4096, 14336, true, "hw_bins/npu2/8192x4096x14336/bf16_int4AWQ_bf16_N"},
            {8192, 14336, 4096, true, "hw_bins/npu2/8192x14336x4096/bf16_int4AWQ_bf16_N"},
        };
        ...
        for (const auto &sweep_case : sweep_cases) {
            ...
            if (sweep_points.empty()) {
                std::cerr << "No valid N-split sweep points found for " << artifact_dir << std::endl;
                continue;
            }
            ...
            for (const auto &point : sweep_points) {
                const std::string result_key = make_result_key(sweep_case, artifact_dir, point.npu_n);
                const bool already_completed =
                    results_doc["results"].contains(result_key) &&
                    results_doc["results"][result_key].value("status", "") == "completed";
                if (already_completed && !force_rerun) {
                    std::cout << "Skipping completed point npuN=" << point.npu_n << std::endl;
                    continue;
                }
            ...
            for (const auto &point : pending_points) {
                const int64_t gpu_n = sweep_case.N - point.npu_n;
                const std::string result_key = make_result_key(sweep_case, artifact_dir, point.npu_n);
                ...
                std::cout << "Running point npuN=" << point.npu_n << " gpuN=" << gpu_n << std::endl;

                try {
                    load_active_context(point);
                    RuntimePointState point_state = prepare_point_state(case_state, sweep_case, point.npu_n);
                    ...
                    point_state.npu_output.zero_();
                    torch::cuda::synchronize();
                    run_split_once(case_state.split_output, case_state.input, point_state, sweep_case.K, sweep_case.N);

                    const auto diff = (case_state.reference_output - case_state.split_output).abs();
                    const double max_abs_diff = diff.max().item<double>();
                    const bool allclose = torch::allclose(case_state.reference_output, case_state.split_output, CHECK_RTOL, CHECK_ATOL);

                    BenchmarkStats stats = benchmark_split(case_state.split_output, case_state.input, point_state, sweep_case.K, sweep_case.N);
                    ...
                    std::cout << "Completed npuN=" << point.npu_n << " avg_latency_us=" << stats.avg_latency_us
                    ...
                    write_results_document(results_path, results_doc);
                    std::cerr << "Point failed for npuN=" << point.npu_n << ": " << point_error.what() << std::endl;
                }
            }
        }
