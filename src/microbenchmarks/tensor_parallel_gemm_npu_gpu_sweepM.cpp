...
std::string make_result_key(const SweepCase &sweep_case, const fs::path &artifact_dir, int64_t npu_m) {
    std::ostringstream oss;
    oss << sweep_case.M << "x" << sweep_case.K << "x" << sweep_case.N << "__" << artifact_dir.filename().string() << "__npuM_" << npu_m;
    return oss.str();
}
...
fs::path get_default_results_path(const std::string &root_dir) {
    return fs::path(root_dir) / "py/heteroMosaic_figs/results/tensor_parallel_gemm_npu_gpu_sweepM_results.json";
}
...
std::vector<SweepPoint> discover_sweep_points(const SweepCase &sweep_case, const fs::path &artifact_dir) {
    ...
    std::map<int64_t, fs::path> pdi_by_npu_m;
    std::map<int64_t, fs::path> inst_by_npu_m;
    ...
    for (const auto &entry : fs::directory_iterator(artifact_dir)) {
        ...
        if (parse_pdi_filename(filename, file_m, file_k, file_n)) {
            if (file_k == sweep_case.K && file_n == sweep_case.N && file_m > 0 && file_m <= sweep_case.M) {
                pdi_by_npu_m[file_m] = entry.path();
            }
        } else if (parse_inst_filename(filename, file_m, file_k, file_n)) {
            if (file_k == sweep_case.K && file_n == sweep_case.N && file_m > 0 && file_m <= sweep_case.M) {
                inst_by_npu_m[file_m] = entry.path();
            }
        }
    }

    std::vector<SweepPoint> points;
    for (const auto &[npu_m, pdi_path] : pdi_by_npu_m) {
        const auto inst_it = inst_by_npu_m.find(npu_m);
        if (inst_it == inst_by_npu_m.end()) {
            continue;
        }
        points.push_back(SweepPoint {npu_m, pdi_path, inst_it->second});
    }

    std::sort(points.begin(), points.end(), [](const SweepPoint &lhs, const SweepPoint &rhs) { return lhs.npu_m < rhs.npu_m; });
    return points;
}
...
std::chrono::microseconds run_gpu_rows(torch::Tensor &output, const torch::Tensor &input, const torch::Tensor &packed_params, int64_t gpu_rows,
                                       int64_t K, int64_t N) {
    if (gpu_rows <= 0) {
        return std::chrono::microseconds(0);
    }

    auto output_slice = output.slice(0, 0, gpu_rows);
    auto input_slice = input.slice(0, 0, gpu_rows);

    const auto start = std::chrono::high_resolution_clock::now();
    hipkernels::w4a16_gemm_fused_packed(output_slice, input_slice, packed_params, K, N);
    torch::cuda::synchronize();
    const auto end = std::chrono::high_resolution_clock::now();
    return std::chrono::duration_cast<std::chrono::microseconds>(end - start);
}

std::chrono::microseconds run_npu_rows(torch::Tensor &output, const torch::Tensor &input, const torch::Tensor &packed_params, int64_t gpu_rows,
                                       int64_t K, int64_t N, uint32_t output_handle, uint32_t input_handle, uint32_t weight_handle) {
    const int64_t npu_rows = output.size(0) - gpu_rows;
    if (npu_rows <= 0) {
        return std::chrono::microseconds(0);
    }

    uint16_t *output_ptr = reinterpret_cast<uint16_t *>(output.data_ptr()) + gpu_rows * N;
    uint16_t *input_ptr = reinterpret_cast<uint16_t *>(input.data_ptr()) + gpu_rows * K;
    void *weight_ptr = packed_params.data_ptr();

    const auto start = std::chrono::high_resolution_clock::now();
    const int ret =
        npuMatmul_zero(ACTIVE_HW_SLOT, ACTIVE_INST_SLOT, output_ptr, input_ptr, weight_ptr, output_handle, input_handle, weight_handle,
                       (hipEvent_t) nullptr);
    const auto end = std::chrono::high_resolution_clock::now();
...
void run_split_once(torch::Tensor &output, const torch::Tensor &input, const torch::Tensor &packed_params, int64_t gpu_rows, int64_t K, int64_t N,
                    uint32_t output_handle, uint32_t input_handle, uint32_t weight_handle) {
    std::chrono::microseconds gpu_time(0);
    std::chrono::microseconds npu_time(0);

    std::optional<std::thread> gpu_thread;
    if (gpu_rows > 0) {
        gpu_thread.emplace([&]() { gpu_time = run_gpu_rows(output, input, packed_params, gpu_rows, K, N); });
        set_thread_affinity(*gpu_thread, GPU_THREAD_CORE);
    }

    std::optional<std::thread> npu_thread;
    if (gpu_rows < input.size(0)) {
        npu_thread.emplace(
            [&]() { npu_time = run_npu_rows(output, input, packed_params, gpu_rows, K, N, output_handle, input_handle, weight_handle); });
...
BenchmarkStats benchmark_split(torch::Tensor &output, const torch::Tensor &input, const torch::Tensor &packed_params, int64_t npu_m, int64_t K,
                               int64_t N, uint32_t output_handle, uint32_t input_handle, uint32_t weight_handle) {
    const int64_t gpu_rows = input.size(0) - npu_m;
    ...
    for (int iter = 0; iter < ITER; ++iter) {
        ...
        run_split_once(output, input, packed_params, gpu_rows, K, N, output_handle, input_handle, weight_handle);
    ...
    const double ops = 2.0 * static_cast<double>(input.size(0)) * static_cast<double>(K) * static_cast<double>(N);
...
void ensure_gpu_baseline_result(json &results_doc, const fs::path &results_path, const SweepCase &sweep_case, const fs::path &artifact_dir,
    ...
    try {
        ...
        torch::cuda::synchronize();
        run_split_once(case_state.split_output, case_state.input, case_state.packed_params, sweep_case.M, sweep_case.K, sweep_case.N,
                       case_state.output_handle, case_state.input_handle, case_state.weight_handle);
        ...
        BenchmarkStats stats = benchmark_split(case_state.split_output, case_state.input, case_state.packed_params, 0, sweep_case.K, sweep_case.N,
                                               case_state.output_handle, case_state.input_handle, case_state.weight_handle);
...
RuntimeCaseState prepare_case_state(const SweepCase &sweep_case) {
    ...
    state.packed_params = build_packed_weights(sweep_case.K, sweep_case.N);
    torch::cuda::synchronize();
    ...
    state.input_handle = import_dma_buf_to_xdna(state.input.data_ptr(), state.input.numel(), state.input.element_size());
    state.output_handle = import_dma_buf_to_xdna(state.split_output.data_ptr(), state.split_output.numel(), state.split_output.element_size());
    state.weight_handle = import_dma_buf_to_xdna(state.packed_params.data_ptr(), state.packed_params.numel(), state.packed_params.element_size());

    if (state.input_handle == 0 || state.output_handle == 0 || state.weight_handle == 0) {
        throw std::runtime_error("Failed to import one or more DMA-BUF handles to XDNA");
    }
...
int main(int argc, char **argv) {
    try {
        ...
        std::vector<SweepCase> sweep_cases = {
            {2048, 2048, 2048, true, "hw_bins/npu2/8192x2048x2048/bf16_int4AWQ_bf16_M"},
            {4096, 4096, 4096, true, "hw_bins/npu2/8192x4096x4096/bf16_int4AWQ_bf16_M"},
            {8192, 8192, 8192, true, "hw_bins/npu2/8192x8192x8192/bf16_int4AWQ_bf16_M"},
            {8192, 4096, 14336, true, "hw_bins/npu2/8192x4096x14336/bf16_int4AWQ_bf16_M"},
            // Intentionally disabled for v1: 4096x14336x4096 lives under bf16_int4AWQ_bf16_K and is not a pure M-split family.
        ...
        for (const auto &sweep_case : sweep_cases) {
            ...
            if (sweep_points.empty()) {
                std::cerr << "No valid M-split sweep points found for " << artifact_dir << std::endl;
                continue;
            }
            ...
            for (const auto &point : sweep_points) {
                const std::string result_key = make_result_key(sweep_case, artifact_dir, point.npu_m);
                const bool already_completed =
                    results_doc["results"].contains(result_key) &&
                    results_doc["results"][result_key].value("status", "") == "completed";
                if (already_completed && !force_rerun) {
                    std::cout << "Skipping completed point npuM=" << point.npu_m << std::endl;
                    continue;
                }
            ...
            for (const auto &point : pending_points) {
                const int64_t gpu_m = sweep_case.M - point.npu_m;
                const std::string result_key = make_result_key(sweep_case, artifact_dir, point.npu_m);
                ...
                std::cout << "Running point npuM=" << point.npu_m << " gpuM=" << gpu_m << std::endl;
                ...
                try {
                    ...
                    case_state.split_output.zero_();
                    torch::cuda::synchronize();
                    run_split_once(case_state.split_output, case_state.input, case_state.packed_params, gpu_m, sweep_case.K, sweep_case.N,
                                   case_state.output_handle, case_state.input_handle, case_state.weight_handle);
                    ...
                    BenchmarkStats stats = benchmark_split(case_state.split_output, case_state.input, case_state.packed_params, point.npu_m,
                                                           sweep_case.K, sweep_case.N, case_state.output_handle, case_state.input_handle,
                                                           case_state.weight_handle);
                    ...
                    std::cout << "Completed npuM=" << point.npu_m << " avg_latency_us=" << stats.avg_latency_us
                    ...
                    write_results_document(results_path, results_doc);
                    std::cerr << "Point failed for npuM=" << point.npu_m << ": " << point_error.what() << std::endl;
                }
            }
        }
